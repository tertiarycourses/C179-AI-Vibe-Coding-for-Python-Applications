...
# ---------------- code rendering ----------------
# A lab "command" is frequently a whole source file, not a one-line shell
# command. A single fixed-height box silently dropped comment-first snippets
# (`# main.py …`) and crushed multi-line ones, so code is wrapped, chunked and
# rendered in a dark card sized to its content.
...
def _wrap_code(cmd):
    ...
    come back at column 0 and read as top-level code. Every pass consumes at
    least one character, so a line with no break opportunity cannot loop forever.
    """
    out=[]
    for raw in cmd.split("\n"):
        ...
        while len(cur)>WRAP:
            ...
            # Prefer a space; dense code often has none, so fall back to a
            # punctuation boundary rather than hard-cutting mid-token.
...
def code_card(s,x,y,w,lines,size=12):
    """Dark card sized to its content, clamped above the footer band."""
    lh=0.235 if size>=12 else 0.205
    h=min(0.34+lh*len(lines), CODE_BOTTOM_LIMIT-(y.inches if hasattr(y,"inches") else CODE_TOP))
    rect(s,x,y,w,Inches(h),CODE_BG,line=DK_FAINT if THEME["dark"] else None)
...
def _explain(cmd, instr):
    ...
    # Pick the framework note by SCORE, not by rule order: `"st." in cmd` also
    # matches inside `pytest.`, so counting occurrences keeps a pytest caption
    # off a Streamlit slide and vice versa.
    import re as _re
    FRAMEWORKS=[
        ("streamlit", r"\bst\.[a-z_]+\(|streamlit", "Streamlit re-runs the whole script on every interaction."),
        ("pytest",    r"\bpytest\b|\bdef test_",     "pytest discovers test_* functions and reports each assertion."),
        ("docker",    r"\bdocker\b|dockerfile",      "The image bundles interpreter, dependencies and code as one artifact."),
    ]
    best=None; bestn=0
    for _name,pat,note in FRAMEWORKS:
        n=len(_re.findall(pat, cmd, _re.I))
        if n>bestn: best,bestn=note,n
    if best: add(best)
    if not out: add("Read every line before you run it — you own the correctness.")
    return out[:5]
...
def step_slide(kicker,act_title,n,total,text,cmd=""):
    ...
    for ci,chunk in enumerate(chunks):
        # Notes describe the code ON THIS SLIDE, so a continuation slide never
        # carries a caption about code that is not on it.
        ...
        oval(s,Inches(0.85),Inches(1.95),Inches(0.82),Inches(0.82),_acc(TEAL))
        txt(s,Inches(0.85),Inches(2.08),Inches(0.82),Inches(0.6),
            [[(str(n),22,DK_BG if THEME["dark"] else WHITE,True)]],align=PP_ALIGN.CENTER)
        txt(s,Inches(1.85),Inches(1.92),Inches(10.6),Inches(0.3),[[(label,11,_grey(),True)]])
        txt(s,Inches(1.85),Inches(2.20),Inches(10.6),Inches(0.62),
            [[(text,19,_ink(),False)]],anchor=MSO_ANCHOR.TOP)
        if not chunk:
            footer(s); continue
        CW=Inches(7.55)                       # code column
        code_card(s,Inches(0.85),Inches(CODE_TOP),CW,chunk,size=12)
        # explanation column
        ex=Inches(8.65); rect(s,ex,Inches(CODE_TOP),Inches(3.85),Inches(3.6),_panel())
        txt(s,ex+Inches(0.28),Inches(3.25),Inches(3.3),Inches(0.35),
            [[("WHAT THIS DOES",11,_acc(BLUE),True)]])
        ...
        for i,note_ in enumerate(notes):
            ...
            r.font.size=Pt(13); r.font.name="Arial"; r.font.color.rgb=_ink()
        footer(s)
    return s

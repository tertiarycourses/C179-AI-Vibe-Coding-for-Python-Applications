...
def collect_links(root):
    ...
    def take_folder(field, folder_key):
        """Link a Drive FOLDER (not a file) — used for the labs tree."""
        canonical, hint = FOLDERS[folder_key]
        try:
            d = find_dir(root, canonical, hint)
        except SystemExit:
            missing.append(f"{FIELD_LABELS[field]}: no '{canonical}' folder in the Drive "
                           f"courseware folder (run /gdrive-push to create and populate it)")
            return
        dirs = rc(["lsjson", f"{REMOTE}:", "--dirs-only"], root, parse=True)
        match = next((x for x in dirs if x["Name"] == d), None)
        if not match:
            missing.append(f"{FIELD_LABELS[field]}: could not resolve the ID of folder '{d}'")
            return
        # empty folder => the link would 'work' but show nothing; that is a silent failure
        if not files_in(root, d) and not rc(["lsjson", f"{REMOTE}:{d}", "--dirs-only"], root, parse=True):
            missing.append(f"{FIELD_LABELS[field]}: Drive folder '{d}' is empty "
                           f"(run /gdrive-push to upload the labs)")
            return
        rc(["link", f"{REMOTE}:{d}"], root)      # anyone-with-link reader permission
        out[field] = (f"{d}/ (folder)",
                      f"https://drive.google.com/drive/folders/{match['ID']}?usp=sharing")
    ...
    # ---- Activities/Lab: the FOLDER itself, not a file. The labs are a tree
    # (topicN/labNN/...), so the LMS links the folder and learners browse it.
    take_folder("activitiesUrl", "activities")
